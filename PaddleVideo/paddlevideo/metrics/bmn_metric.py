    ...
    while len(tscore) > 1 and len(rscore) < 101:
        ...
        for idx in range(0, len(tscore)):
            if idx != max_index:
                ...
                tmp_center = (tend[max_index] + tstart[max_index])/2
                center = (tend[idx] + tstart[idx])/2
                distance = abs(tmp_center - center)
                cover_distance = abs(max(tend[idx],tend[max_index])-min(tstart[idx],tstart[max_index]))
                tmp_width = tend[max_index] - tstart[max_index]
                tmp_diou = tmp_iou - (distance/cover_distance)**2
                if tmp_diou > t1 + (t2 - t1) * tmp_width:
                    tscore[idx] = tscore[idx] * np.exp(
                        -np.square(tmp_diou) / alpha)
